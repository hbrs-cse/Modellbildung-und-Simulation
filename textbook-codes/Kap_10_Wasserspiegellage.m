...
% Wasserspiegellagenberechnung als Randwertproblem
 global g B Ks uL hR
  ...
  % grobes Ortsgitter
  L = 10000; N = 10; x = linspace(0,L,N);
  solinit = bvpinit(x,@winit);
  sol = bvp4c(@dgl,@bc,solinit);
  % Wasserspiegelhoehe
  z = sol.y(1,:) + S_0(sol.x);
  % Feines Ortsgitter
  xx = linspace(0,L,1000);  zz = deval(sol,xx);
  subplot(2,1,1);
  plot(xx,S_0(xx),sol.x,z,'o',xx,zz(1,:)+S_0(xx),'linewidth',2); 
  legend('S_0(x)','z(x)');
  xlabel('Fliessstrecke'); ylabel('Sohl- u. Wasserspiegelhoehe');
  ax = gca; ax.FontSize = 11;
  subplot(2,1,2); plot(xx,zz(2,:),'linewidth',2); 
  legend('u(x)');
  xlabel('Fliessstrecke'); ylabel('Fliessgeschwindigkeit');
end
