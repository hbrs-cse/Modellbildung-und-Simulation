...
quiver(x_RK(1),                     y_RK(1),                    dt_RK(1)*6,     df_RK(1)*6,'off','linewidth',1,'color','r','DisplayName','K_1 = Euler step 1');
% legend('dy/dx(y_i,x_i)','ideal function','RK solution','Euler solution','K_1 = Euler');
printgif(h,filename)
quiver(x_RK(2),                     y_RK(2),                    dt_RK(2)*6/4,   df_RK(2)*6/4,'off','linewidth',1,'color','g','DisplayName','K_2 @u_m + dt*K_1/2');
% legend('dy/dx(y_i,x_i)','ideal function','RK solution','Euler solution','K_1 = Euler','K_2');
printgif(h,filename)
quiver(x_RK(1),                     y_RK(1),                   -dt_RK(1)*6,    -df_RK(1)*6,'off','linewidth',1,'color','r','DisplayName','t_i - dt, u_m + dt*(-K_1)');
printgif(h,filename)
quiver(x_RK(1)-dt_RK(1)*6,            y_RK(1)-df_RK(1)*6,           dt_RK(2)*6/4*2, df_RK(2)*6/4*2,'off','linewidth',1,'color','g','DisplayName','t_i + dt, u_m + dt*(-K_1+2*K_2)');
printgif(h,filename)
quiver(x_RK(3),                     y_RK(3),                    dt_RK(3)*6,     df_RK(3)*6,'off','linewidth',1,'color','b','DisplayName','K_3 @u_m + dt*(-K-1+K_2');
% legend('dy/dx(y_i,x_i)','RK solution','Euler solution','ideal function','K_1 = Euler','K_2','K_3');
printgif(h,filename)

%% adding K_i together
quiver(x_RK(1),                     y_RK(1),                    dt_RK(1),       df_RK(1),'off','linewidth',1,'color','r','DisplayName','1/6 K_1');
% legend('dy/dx(y_i,x_i)','RK solution','Euler solution','ideal function','K_1 = Euler','K_2','K_3','');
printgif(h,filename)
quiver(x_RK(1)+dt_RK(1),            y_RK(1)+df_RK(1),           dt_RK(2),       df_RK(2),'off','linewidth',1,'color','g','DisplayName','1/6 K_1, 4/6 K_2');
% legend('dy/dx(y_i,x_i)','RK solution','Euler solution','ideal function','K_1 = Euler','K_2','K_3','');
printgif(h,filename)
quiver(x_RK(1)+dt_RK(1)+dt_RK(2),   y_RK(1)+df_RK(1)+df_RK(2),  dt_RK(3),       df_RK(3),'off','linewidth',1,'color','b','DisplayName','1/6 K_1, 4/6 K_2, 1/6 K_3');
% legend('dy/dx(y_i,x_i)','RK solution','Euler solution','ideal function','K_1 = Euler','K_2','K_3','','');
printgif(h,filename)
%% final step
quiver(x_RK(4),y_RK(4),dt_RK(4),df_RK(4),'off','linewidth',1.25,'color','m','DisplayName','RK step 1');
...
function [u_m_new,err] = RK(ti,dt,u_m,fdot)
    ...
    u_m3 = u_m + dt*(K_1/6 + K_2*4/6 + K_3l/6);
